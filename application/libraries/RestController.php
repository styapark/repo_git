...
class RestController extends \CI_Controller
{
    ...
    protected function _force_login($nonce = '')
    {
        ...
        if ( $this->config->item('force_ux') ) {
            $res = $this->load->view('errors/html/error_general',[
                'heading'=> self::HTTP_UNAUTHORIZED.' '.$this->lang->line('text_rest_unauthorized'),
                'message'=>'<p>Access to this resource is denied, your client has not supplied the correct authentication.</p>'
            ], TRUE);
            die($res);
        }
        // Display an error response
